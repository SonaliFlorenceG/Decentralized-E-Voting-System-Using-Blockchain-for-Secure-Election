...
export const VotingInterface: React.FC<VotingInterfaceProps> = ({
  ...
  const sendOtp = async () => {
    ...
    try {
      const res = await fetch(
        "https://decentralized-e-voting-system-using.onrender.com/send-otp",
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ phoneNumber: formattedPhone }),
  ...
  const verifyOtp = async () => {
    ...
    console.log("📤 Verifying:", formattedPhone, otp);
    ...
    try {
      const res = await fetch(
        "https://decentralized-e-voting-system-using.onrender.com/verify-otp",
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
  ...
  return (
    <div className="p-6">
      ...
      {/* OTP MODAL */}
      <Transition appear show={isOtpModalOpen} as={Fragment}>
        <Dialog onClose={() => setIsOtpModalOpen(false)}>
